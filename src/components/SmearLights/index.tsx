...
import { Popup, Slider } from "@ray-js/smart-ui";
import { useActions, useProps, useDevice, utils, useStructuredProps, useStructuredActions } from '@ray-js/panel-sdk';
...
import { selectCollectSmearMap, updateCollectSmearMap, selectSmearLightColorMaps, updateSmearLightColorMaps } from '@/redux/modules/cloudStateSlice'
...
const SmearLights = () => {
  ...
  const smearedColor = useMemo(() => {
    ...
    const rgbString = brightKelvin2rgb(smearData.brightness === 0 ? 0 : 1000, smearData.temperature);
    const rgbArray = rgbString
      .match(/\d+/g) // 使用正则表达式提取所有数字
      ?.map(Number)
    const hsv: number[] = rgb2hsv(rgbArray[0], rgbArray[1], rgbArray[2]).map(item => Math.round(item));
    ...
    if (index === 0) {
      ...
      Object.keys(smearLightColorMaps).forEach((key) => {
        if (smearData.dimmerMode === DimmerMode.colour) {
          ...
          _lightColorMaps[key] = brightKelvin2rgb(smearData.brightness === 0 ? 0 : 1000, smearData.temperature);
        }
      });
    ...
    [...data].forEach(item => {
      if (smearData.smearMode === SmearMode.clear) {
        ...
        if (smearData.dimmerMode === DimmerMode.colour) {
          ...
          newCheckedMapColor[item] = brightKelvin2rgb(smearData.brightness === 0 ? 0 : 1000, smearData.temperature);
        }
      }
    });
    const _lightColorMaps = {
  ...
  const updateCloudSmearLightColorMapsAsync = (data: Record<string, string>) => {
    ...
    return Promise.all(
      chunks.map((chunk, index) => {
        return devices.lamp.model.abilities.storage.set(CLOUD_DATA_KEYS_MAP.smearLightColorMaps + "_" + index, chunk)
      })
    ).then(() => {
