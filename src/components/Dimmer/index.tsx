  ...
  const updateCloudSmearLightColorMapsAsync = (data: Record<string, string>) => {
    ...
    return Promise.all(
      chunks.map((chunk, index) =>
        devices.lamp.model.abilities.storage.set(
  ...
  const mainLightRander = useMemo(() => {
    ...
    return (
      <View className={clsx(!main_switch && styles.offLight)}>
        {tabList.filter(item => !item.hide).length > 1 && (
  ...
  const auxLightRander = useMemo(() => {
    ...
    return (
      <View className={clsx(!aux_switch && styles.offLight)}>
        {auxTabList.filter(item => !item.hide).length > 1 && (
          <View className={styles.tab}>
            {auxTabList
              .filter(item => !item.hide)
              .map((item, index) => (
                <View
                  key={index}
                  className={clsx(styles.tab_item, auxTabIndex === index && styles.tab_active)}
                  onClick={() => {
        ...
        {aux_mode === 'white' && (
          <WhiteLightPanel
            ...
            onChoosePrimaryWhite={index => {
              ...
              action.aux_temp.set(Math.round(_white.temperature));
            }}
            handleTouchEndByWhite={value => {
              action.aux_temp.set(value);
            }}
            onChangeBrightness={value => {
