...
import { selectModeIndex, updateModeIndex, selectColorMode, updateColorMode } from '@/redux/modules/uiStateSlice';
...
export function Home() {
  ...
  const BannerRander = useMemo(() => {
    if (power) {
      if (main_switch && aux_switch) {
        return <Image className={styles.banner} src={require('@/static/images/home/all_light.png')} />
      } else if (main_switch) {
        return <Image className={styles.banner} src={require('@/static/images/home/king_light.png')} />
      } else if (aux_switch) {
        return <Image className={styles.banner} src={require('@/static/images/home/aux_light.png')} />
      } else {
  ...
  return (
    <View className={styles.view}>
      <ScrollView className={styles.scroll_view} scrollY={true}>
        <View className={styles.content}>
          {BannerRander}
          {/* 开关 */}
          <View className={styles.switch}>
            <View className={clsx(styles.switch_box)}
              style={{
                marginLeft: "16rpx",
          ...
          {power && <>
            <View className={styles.tabbar}>
              {tabList.map((item, index) => (
                <View key={item.title} className={clsx(styles.tabbar_item, modeIndex === index && styles.tabbar_item_active)}
                  onClick={() => {
